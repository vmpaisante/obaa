...
void Address::setBase(RangedPointer* b)
{
  base = b;
}

void Address::setOffset(Offset o)
{
  offset = o;
}
...
void Address::Expand (std::deque<Address*> &adq, std::set<Address*> &fn)
{
  ...
  if(expanded.find(base) == expanded.end())
  {
    ...
    for(auto i = aux.begin(), ie = aux.end(); i != ie; i++)
    {
      ...
      else if(ad->argument) newAddress->argument = true;
      ...
      if(widened) newAddress->widened = true;
      else if(ad->widened) newAddress->widened = true;
      ...
      for(auto i : ad->narrowingOps)
        if(newAddress->narrowingOps.find(i.first) == newAddress->narrowingOps.end())
          newAddress->narrowingOps[i.first] = i.second.contextualize(offset);
      
      newAddress->wideningOps = wideningOps;
      for(auto i : ad->wideningOps)
        if(newAddress->wideningOps.find(i.first) == newAddress->wideningOps.end())
